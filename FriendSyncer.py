...
minecraftdir = input("Input .minecraft's directory. \nEx) C:\\Users\\meltd\\AppData\\Roaming\\.minecraft \nInput : ")
friendList = []
futuredir = ""
nameUUID={}
    ...
    if ("Future" in os.listdir()):
        try:
            with open("{0}/Future/friends.json".format(userprofiledir)) as futureFile:
                ...
                print("Found Future Client folder : {0}\\Future, Grabbing Friends...".format(userprofiledir))
                addcnt = 0
                ...
                print("Added {0} {1} in FriendList from Future".format(addcnt, fname),end="")
            ...
            with open("{0}/rusherhack/friends.json".format(minecraftdir)) as rusherFile:
                ...
                for i in range(len(friendRJson)):
                    ...
                    if (friendNick not in friendList):
                        ...
                        addcnt += 1
                fname =  "friend" if addcnt<=1 else "friends"
                print("Added {0} {1} in FriendList from Rusher".format(addcnt, fname),end="")
        ...
        try:
            with open("{0}/lambda/friends.json".format(minecraftdir)) as lambdaFile:
                ...
                for i in range(len(friendLJson["Friends"])):
                    nameUUID[friendLJson["Friends"][i]['name']]=friendLJson["Friends"][i]['uuid']
                    friendNick = friendLJson["Friends"][i]['name']
                    if (friendNick not in friendList):
                        friendList.append(friendNick)
                        addcnt += 1
                fname =  "friend" if addcnt<=1 else "friends"
                print("Added {0} {1} in FriendList from Lambda".format(addcnt, fname),end="")
                detectedClient["Lambda"] = True
                if (addcnt == len(friendLJson["Friends"])):
                    print(".")
                else:
                    print(", {0} overlapped.".format(len(friendLJson["Friends"])-addcnt))
                print("")
        except Exception as e:
            print("Error : {0}\n".format(e))
...
if (detectedClient["Lambda"]):
    ...
    for i in chunks:
        ...
        for name, uuid in uuids.items():
            nameUUID[name]=uuid[:8]+"-"+uuid[8:12]+"-"+uuid[12:16]+"-"+uuid[16:20]+"-"+uuid[20:]
        print("Grabbing UUID from MojangAPI...({0}/{1})".format(cnt,len(chunks)))
    ...
    for i in errorList:
        if (i.lower() in addList):
            print("Can't Find {0}'s UUID, but there are same nickname in FriendList with only lower/uppercase Difference, it seems like overlapped.".format(i))
        else:
            print("Can't Find {0}'s UUID, Please update to Recent Nickname, this friend will be ignored in add List.".format(i))
